...
public class Tank_Fire_Collision : MonoBehaviour {
	...
	void OnParticleCollision(GameObject other) {
		...
		if (other.CompareTag ("Player")){
			...
			if(!player.GetComponent<Tank_Fire_Collision>().isActiveAndEnabled)
			{
			player.AddComponent<Tank_Fire_Collision>();
			player.GetComponent<Tank_Fire_Collision>().fireHit=true;
			player.GetComponent<Tank_Fire_Collision>().dirtMaterial=dirtMaterial;
			}
	...
	void Update () {
		if(fireHit)
		{
			...
			if(Explosion_Game_OverTime<0.0f)
			{
				...
				player.GetComponent<score_n_time>().ClockStop=true;
			}
